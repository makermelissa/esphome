...
std::string format_ip4_addr(esp_ip4_addr_t &ip) {
#else
...
#if ESP_IDF_VERSION_MAJOR >= 4
      auto it = info.wifi_sta_connected;
      ESP_LOGV(TAG, "Event: AP client connected MAC=%s aid=%u", format_mac_addr(it.bssid).c_str(), it.ssid);
#else
...
#if ESP_IDF_VERSION_MAJOR >= 4
      auto it = info.wifi_sta_disconnected;
      ESP_LOGV(TAG, "Event: AP client disconnected MAC=%s aid=%u", format_mac_addr(it.bssid).c_str(), it.ssid);
#else
